...
describe('Testing Toy Routes', function () {
  ...
  describe('DELETE requests', function () {
    ...
    describe('Valid Requests', () => {
      beforeAll(() => {
        return superagent.delete(`:3000/api/toy/${this.toy._id}`)
      ...
      test('should remove the Toy from the DB', () => {
        return superagent.get(`:3000/api/toy/${this.toy._id}`)
        .catch(err => {
    ...
    describe('Invalid Requests', () => {
      test('should return a 404 Not Found given bad Id in route', () => {
        return superagent.delete(':3000/api/toy/badId')
        .catch(err => {
          expect(err.status).toBe(404)
        })
      })
      test('should return a 404 Not Found given bad route', () => {
        return superagent.delete(':3000/api/bad/route')
        .catch(err => {
          expect(err.status).toBe(404)
        })
      })

  ...
  describe('DELETE requests', function () {
    ...
    describe('Valid Requests', () => {
      test('should return a status 204 No Content', () => {
        return superagent.delete(`:3000/api/child/${this.children[0]._id}`)
        .then(res => {
      ...
      test('should remove a record from the db', () => {
        return superagent.delete(`:3000/api/child/${this.children[1]._id}`)
        .then(() => {
          return superagent.get(`:3000/api/child/${this.children[1]._id}`)
          .catch(err => {
    ...
    describe('Invalid Requests', () => {
      test('should return a 404 Not Found given an invalid Id', () => {
        return superagent.delete(`:3000/api/child/badId`)
        .catch(err => {
          expect(err.status).toBe(404)
        })
      })

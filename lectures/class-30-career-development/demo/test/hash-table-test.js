...
describe('Hash Table Module', function() {
  before(done => {
    this.fakeKeys = [...Array(100)].map(ele => ele = faker.hacker.phrase())
  ...
  describe('Hash a key for the Hash Table: ', () => {
    it('should hash a key', done => {
      ...
      expect(expectHash).to.equal(1225)
      done()
    })
